...
const idleResource = { data: null, loading: false, error: '' }

function asMessage(err, fallback) {
  return err?.message || fallback
}

export function useProgress(toast) {
  const [summary, setSummary] = useState(idleResource)
  const [history, setHistory] = useState(idleResource)
  const [lifts, setLifts] = useState(idleResource)
  const [body, setBody] = useState(idleResource)
  const [records, setRecords] = useState(idleResource)
  const [compare, setCompare] = useState(idleResource)
  const [lifestyle, setLifestyle] = useState(idleResource)
  ...
  const loadLifts = useCallback(async (opts = {}) => {
  ...
  const loadBody = useCallback(async (opts = {}) => {
  ...
  const loadCompare = useCallback(async (series, opts = {}) => {
  ...
  const logBodyMetric = useCallback(async (payload) => {
    try {
