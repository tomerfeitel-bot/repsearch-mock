const { db, getAll, runQuery } = require('./db')
...
function runExerciseProfileForUser(userId) {
  ...
  for (const [exerciseId, exSets] of byExercise) {
    ...
    weeks.forEach((w, i) => {
      ...
      const rowId = nanoid()
      const existing = db.prepare('SELECT id FROM user_exercise_profile WHERE user_id = ? AND exercise_id = ? AND week = ?').get([userId, exerciseId, w])
      const cols = [
...
function runSystemicProfileForUser(userId) {
  const dailyRows = getAll('SELECT * FROM daily_log WHERE user_id = ? ORDER BY date ASC', [userId])
  const activityRows = getAll('SELECT * FROM activity_log WHERE user_id = ? ORDER BY date ASC', [userId])
  const workoutRows = getAll('SELECT date FROM workouts WHERE user_id = ? ORDER BY date ASC', [userId])
  ...
  for (const a of activityRows) {
    ...
    if (a.activity_type === 'running') b.cardio.run += load
    else if (a.activity_type === 'cycling') b.cardio.cyc += load
    else if (a.activity_type === 'swimming') b.cardio.swm += load
    else b.cardio.oth += load
  }
  ...
  for (const [week, b] of byWeek) {
    ...
    const existing = db.prepare('SELECT id FROM user_systemic_profile WHERE user_id = ? AND week = ?').get([userId, week])
    const cols = [
...
function runWeeklyBatch() {
  ...
  try {
    for (const u of users) {
      try { runExerciseProfileForUser(u.id) } catch (e) { console.error('uep failed for', u.id, e.message) }
      try { runSystemicProfileForUser(u.id) } catch (e) { console.error('usp failed for', u.id, e.message) }
    }
    db.exec('COMMIT')
  } catch (err) {
