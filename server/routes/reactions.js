...
router.post('/', authRequired, async (req, res) => {
  ...
  const workout = await getOne('SELECT id, visibility, user_id FROM workouts WHERE id = ?', [workout_id]);
  if (!workout) return res.status(404).json({ error: 'Workout not found' });
  if (!(await canViewWorkout(workout, req.user.id))) return res.status(403).json({ error: 'Forbidden' });
...
router.get('/workout/:workoutId', authRequired, async (req, res) => {
  const workout = await getOne('SELECT id, visibility, user_id FROM workouts WHERE id = ?', [req.params.workoutId]);
  if (!workout) return res.status(404).json({ error: 'Workout not found' });
  if (!(await canViewWorkout(workout, req.user.id))) return res.status(403).json({ error: 'Forbidden' });
