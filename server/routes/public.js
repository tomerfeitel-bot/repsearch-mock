...
router.get('/users/:username', authOptional, (req, res) => {
  const user = userWithDerivedFields(getOne(
  ...
  const followsThem = req.user
    ? !!getOne('SELECT 1 AS x FROM follows WHERE follower_id = ? AND following_id = ?', [req.user.id, user.id])
    : false
  ...
  const followerCount = getOne('SELECT COUNT(*) AS n FROM follows WHERE following_id = ?', [user.id]).n
  const followingCount = getOne('SELECT COUNT(*) AS n FROM follows WHERE follower_id = ?', [user.id]).n
  const workoutCount = getOne('SELECT COUNT(*) AS n FROM workouts WHERE user_id = ?', [user.id]).n

  // Weekly training frequency derived from the last 28 days of logged workouts.
  const recentWorkouts = getOne(
  ...
  const publishedPrograms = getAll(
    `SELECT p.*,
...
router.get('/workouts/:id', authOptional, (req, res) => {
  const w = getOne('SELECT * FROM workouts WHERE id = ?', [req.params.id])
  if (!w) return res.status(404).json({ error: 'Workout not found' })
  if (!canViewWorkout(w, req.user?.id)) return res.status(403).json({ error: 'Forbidden' })
  const owner = getOne('SELECT id, username, bio FROM users WHERE id = ?', [w.user_id])
  const sets = getAll(
