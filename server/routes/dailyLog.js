...
function syncBodyweight(userId, date, bodyweightKg, now) {
  ...
  if (existing) {
    runQuery('UPDATE body_metrics_history SET bodyweight_kg = ?, created_at = ? WHERE id = ?', [bodyweightKg, now, existing.id])
  } else {
    runQuery(
...
router.post('/', authRequired, (req, res) => {
  ...
  const existing = getOne('SELECT id FROM daily_log WHERE user_id = ? AND date = ?', [userId, date])
  const now = nowIso()
  if (existing) {
    const cols = Object.keys(cleaned).map(k => `${k} = ?`)
    if (cols.length) {
      runQuery(`UPDATE daily_log SET ${cols.join(', ')} WHERE id = ?`, [...Object.values(cleaned), existing.id])
    }
    if (Object.prototype.hasOwnProperty.call(cleaned, 'bodyweight_kg')) {
      syncBodyweight(userId, date, cleaned.bodyweight_kg, now)
    }
    res.json({ log: getOne('SELECT * FROM daily_log WHERE id = ?', [existing.id]) })
  } else {
    const id = nanoid()
    ...
    res.json({ log: getOne('SELECT * FROM daily_log WHERE id = ?', [id]) })
  }
})
