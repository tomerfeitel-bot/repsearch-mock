...
function rowsForUser(userId, query = {}) {
  ...
  if (query.set_type) { wheres.push('s.set_type = ?'); params.push(String(query.set_type)) }
  else {
...
function workoutRows(userId, query = {}) {
...
function setsForWorkoutIds(userId, ids) {
...
function bodyHistory(userId, query = {}) {
...
router.get('/summary', authRequired, (req, res) => {
  const workouts = workoutRows(req.user.id)
  const now = new Date()
...
router.get('/history', authRequired, (req, res) => {
  const workouts = workoutRows(req.user.id, req.query)
  const sets = setsForWorkoutIds(req.user.id, workouts.map(w => w.id))
...
router.get('/lifts', authRequired, (req, res) => {
  const rows = rowsForUser(req.user.id, req.query)
...
router.get('/body', authRequired, (req, res) => {
  const history = bodyHistory(req.user.id, req.query)
...
router.get('/compare', authRequired, (req, res) => {
  ...
  const groupBy = cleanGroup(req.query.group_by)
  const results = seriesDefs.map((definition, index) => {
    ...
    if (sourceType === 'body_metric') {
      ...
      const points = [...bodyHistory(req.user.id, req.query)]
        .reverse()
    ...
    const rows = rowsForUser(req.user.id, query)
    return {
