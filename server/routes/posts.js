...
function followingIds(userId) {
  return getAll('SELECT following_id FROM follows WHERE follower_id = ?', [userId]).map(r => r.following_id)
}

function labelsFor(postId) {
  return getAll('SELECT label FROM post_labels WHERE post_id = ?', [postId]).map(r => r.label)
}

function recomputePostScore(postId) {
  const sum = getOne('SELECT COALESCE(SUM(value), 0) AS s FROM post_votes WHERE post_id = ?', [postId]).s
  dbRun('UPDATE posts SET score = ? WHERE id = ?', [sum, postId])
  return sum
}

function recomputeCommentScore(commentId) {
  const sum = getOne('SELECT COALESCE(SUM(value), 0) AS s FROM comment_votes WHERE comment_id = ?', [commentId]).s
  dbRun('UPDATE comments SET score = ? WHERE id = ?', [sum, commentId])
  return sum
}
...
function attachmentSummary(post) {
  ...
  if (post.kind === 'study') {
    return hydrateStudy(post.study_feature_json)
  }
  return null
}

function runStudySide(feature, filters) {
  const r = researchQuery(db, {
    filters,
...
function hydrateStudy(json) {
  ...
  if (base.mode === 'compare') {
    const a = runStudySide(feature, feature.cohortA?.filters || [])
    const b = runStudySide(feature, feature.cohortB?.filters || [])
    value = {
      ...base,
    ...
    value = { ...base, ...runStudySide(feature, feature.filters || []) }
  }
...
function shapePost(row, userId, { withAttachment = true } = {}) {
  const vote = getOne('SELECT value FROM post_votes WHERE post_id = ? AND user_id = ?', [row.id, userId])
  const saved = getOne('SELECT 1 AS x FROM saved_posts WHERE post_id = ? AND user_id = ?', [row.id, userId])
  return {
    id: row.id,
    kind: row.kind,
    title: row.title,
    ...
    saved: !!saved,
    attachment: withAttachment ? attachmentSummary(row) : undefined,
  }
}
...
function shapePostsBatch(rows, userId) {
  if (!rows.length) return []
  const ids = rows.map(r => r.id)
  const ph = ids.map(() => '?').join(',')

  const voteMap = new Map(
    getAll(`SELECT post_id, value FROM post_votes WHERE user_id = ? AND post_id IN (${ph})`, [userId, ...ids])
      .map(r => [r.post_id, r.value]))
  const savedSet = new Set(
    getAll(`SELECT post_id FROM saved_posts WHERE user_id = ? AND post_id IN (${ph})`, [userId, ...ids])
      .map(r => r.post_id))
  const labelsMap = new Map()
  for (const r of getAll(`SELECT post_id, label FROM post_labels WHERE post_id IN (${ph})`, ids)) {
  ...
  const fetchSummaries = (kind, sql) => {
    ...
    const byId = new Map(getAll(sql.replace('__IN__', aph), aids).map(x => [x.id, x]))
    for (const r of group) attByPost.set(r.id, byId.get(r.attachment_id) || null)
  }
  fetchSummaries('workout',
    `SELECT w.id, w.date, w.duration_min, w.workout_day, w.workout_split_type,
            (SELECT COUNT(*) FROM sets s WHERE s.workout_id = w.id) AS set_count,
            (SELECT COUNT(DISTINCT s.exercise_id) FROM sets s WHERE s.workout_id = w.id) AS exercise_count
       FROM workouts w WHERE w.id IN (__IN__)`)
  fetchSummaries('program',
    `SELECT p.id, p.name, p.description, p.weeks, p.strictness, p.visibility,
            (SELECT COUNT(*) FROM program_enrollments WHERE program_id = p.id) AS enrollment_count
       FROM programs p WHERE p.id IN (__IN__)`)
  fetchSummaries('template',
    `SELECT t.id, t.name, t.description, t.workout_day, t.usage_count, t.strictness, t.visibility,
            (SELECT COUNT(*) FROM template_exercises te WHERE te.template_id = t.id) AS exercise_count
       FROM workout_templates t WHERE t.id IN (__IN__)`)
  for (const r of rows) {
    if (r.kind === 'study') attByPost.set(r.id, hydrateStudy(r.study_feature_json))
  }
...
router.post('/', authRequired, (req, res) => {
  ...
  if (ITEM_KINDS[kind]) {
    ...
    const item = getOne(`SELECT * FROM ${table} WHERE id = ?`, [id])
    ...
    if (visibility === 'public') {
      dbRun(`UPDATE ${table} SET visibility = 'public' WHERE id = ?`, [id])
    } else {
      // followers post: only lift a fully-private item up to 'followers'.
      dbRun(`UPDATE ${table} SET visibility = 'followers' WHERE id = ? AND visibility = 'private'`, [id])
    }
    attachmentType = kind
    attachmentId = id
  } else if (kind === 'study') {
  ...
  try {
    ...
    for (const label of labels) {
      dbRun('INSERT OR IGNORE INTO post_labels (post_id, label) VALUES (?, ?)', [id, label])
    }
    db.exec('COMMIT')
  } catch (err) {
  ...
  const row = getOne('SELECT p.*, u.username FROM posts p JOIN users u ON u.id = p.user_id WHERE p.id = ?', [id])
  res.status(201).json({ post: shapePost(row, userId) })
})
...
router.get('/', authRequired, (req, res) => {
  ...
  const params = []
  if (scope === 'following') {
    const ids = [...followingIds(userId), userId]
  ...
  res.json({
    items: shapePostsBatch(paged, userId),
...
router.get('/saved', authRequired, (req, res) => {
  ...
  res.json({ items: shapePostsBatch(rows, userId) })
})
...
router.get('/compose-options', authRequired, (req, res) => {
  const userId = req.user.id
...
router.post('/comments/:commentId/vote', authRequired, (req, res) => {
  const value = req.body?.value === -1 ? -1 : req.body?.value === 1 ? 1 : 0
  const comment = getOne('SELECT id FROM comments WHERE id = ?', [req.params.commentId])
  if (!comment) return res.status(404).json({ error: 'Comment not found' })
  const existing = getOne('SELECT value FROM comment_votes WHERE comment_id = ? AND user_id = ?', [req.params.commentId, req.user.id])
  if (value === 0 || (existing && existing.value === value)) {
    dbRun('DELETE FROM comment_votes WHERE comment_id = ? AND user_id = ?', [req.params.commentId, req.user.id])
  } else if (existing) {
    dbRun('UPDATE comment_votes SET value = ? WHERE comment_id = ? AND user_id = ?', [value, req.params.commentId, req.user.id])
  } else {
    dbRun('INSERT INTO comment_votes (comment_id, user_id, value) VALUES (?, ?, ?)', [req.params.commentId, req.user.id, value])
  }
  const score = recomputeCommentScore(req.params.commentId)
  res.json({ score, viewer_vote: getOne('SELECT value FROM comment_votes WHERE comment_id = ? AND user_id = ?', [req.params.commentId, req.user.id])?.value || 0 })
})

// --- detail -----------------------------------------------------------------
router.get('/:id', authRequired, (req, res) => {
  const userId = req.user.id
  const row = getOne('SELECT p.*, u.username FROM posts p JOIN users u ON u.id = p.user_id WHERE p.id = ?', [req.params.id])
  if (!row) return res.status(404).json({ error: 'Post not found' })
  if (row.visibility === 'followers' && row.user_id !== userId) {
    const f = getOne('SELECT 1 AS x FROM follows WHERE follower_id = ? AND following_id = ?', [userId, row.user_id])
    if (!f) return res.status(403).json({ error: 'Forbidden' })
  }
  res.json({ post: shapePost(row, userId), comments: commentTree(req.params.id, userId) })
})

function commentTree(postId, userId) {
  const rows = getAll(
  ...
  const votes = new Map(
    getAll('SELECT comment_id, value FROM comment_votes WHERE user_id = ? AND comment_id IN (SELECT id FROM comments WHERE post_id = ?)', [userId, postId])
...
router.post('/:id/vote', authRequired, (req, res) => {
  ...
  const existing = getOne('SELECT value FROM post_votes WHERE post_id = ? AND user_id = ?', [req.params.id, req.user.id])
  if (value === 0 || (existing && existing.value === value)) {
    dbRun('DELETE FROM post_votes WHERE post_id = ? AND user_id = ?', [req.params.id, req.user.id])
  } else if (existing) {
    dbRun('UPDATE post_votes SET value = ? WHERE post_id = ? AND user_id = ?', [value, req.params.id, req.user.id])
  } else {
    dbRun('INSERT INTO post_votes (post_id, user_id, value) VALUES (?, ?, ?)', [req.params.id, req.user.id, value])
  }
  const score = recomputePostScore(req.params.id)
  res.json({ score, viewer_vote: getOne('SELECT value FROM post_votes WHERE post_id = ? AND user_id = ?', [req.params.id, req.user.id])?.value || 0 })
})

router.post('/:id/save', authRequired, (req, res) => {
  const post = getOne('SELECT id FROM posts WHERE id = ?', [req.params.id])
  if (!post) return res.status(404).json({ error: 'Post not found' })
  dbRun('INSERT OR IGNORE INTO saved_posts (post_id, user_id, created_at) VALUES (?, ?, ?)', [req.params.id, req.user.id, nowIso()])
  res.json({ saved: true })
})

router.delete('/:id/save', authRequired, (req, res) => {
  dbRun('DELETE FROM saved_posts WHERE post_id = ? AND user_id = ?', [req.params.id, req.user.id])
...
router.post('/:id/comments', authRequired, (req, res) => {
  ...
  if (parentId) {
    const parent = getOne('SELECT id FROM comments WHERE id = ? AND post_id = ?', [parentId, req.params.id])
  ...
  dbRun('UPDATE posts SET comment_count = (SELECT COUNT(*) FROM comments WHERE post_id = ?) WHERE id = ?', [req.params.id, req.params.id])
  res.status(201).json({
