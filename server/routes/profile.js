...
async function patchUser(req, res) {
  ...
  if (cols.length === 0) return res.json({ user: userWithDerivedFields(await getOne('SELECT * FROM users WHERE id = ?', [req.user.id])) });

  vals.push(req.user.id);
  await runQuery(`UPDATE users SET ${cols.join(', ')} WHERE id = ?`, vals);

  const fresh = userWithDerivedFields(await getOne('SELECT * FROM users WHERE id = ?', [req.user.id]));
  const allMandatorySet = MANDATORY_FIELDS.every((f) => fresh[f] !== null && fresh[f] !== undefined && fresh[f] !== '');
  if (allMandatorySet && !fresh.onboarded) {
    await runQuery('UPDATE users SET onboarded = 1 WHERE id = ?', [req.user.id]);
    fresh.onboarded = 1;
  }
  res.json({ user: fresh });
}
...
router.post('/advanced', authRequired, async (req, res) => {
