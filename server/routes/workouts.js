...
async function loadWorkoutWithSets(workoutId, userId) {
  const workout = await getOne('SELECT * FROM workouts WHERE id = ?', [workoutId]);
  if (!workout) return null;
  if (!(await canViewWorkout(workout, userId))) return null;
...
router.get('/', authRequired, async (req, res) => {
  ...
  const total = (await getOne(`SELECT COUNT(*) AS n FROM workouts WHERE ${wheres.join(' AND ')}`, params)).n;
...
router.get('/:id', authRequired, async (req, res) => {
  const w = await loadWorkoutWithSets(req.params.id, req.user.id);
...
router.patch('/:id', authRequired, async (req, res) => {
  ...
  if (!cols.length) return res.json({ workout: await loadWorkoutWithSets(req.params.id, req.user.id) });
  vals.push(req.params.id);
  await runQuery(`UPDATE workouts SET ${cols.join(', ')} WHERE id = ?`, vals);
  res.json({ workout: await loadWorkoutWithSets(req.params.id, req.user.id) });
});
