...
router.get('/start-suggestions', authRequired, (req, res) => {
...
router.get('/:id', authRequired, (req, res) => {
  const t = loadTemplateFull(req.params.id)
  if (!t) return res.status(404).json({ error: 'Template not found' })
  const isOwner = t.user_id === req.user.id
  const isPublic = t.visibility === 'public'
  const viaEnrollment = !isOwner && !isPublic && !!getOne(
...
function replaceTemplateExercises(templateId, exercises) {
  runQuery('DELETE FROM template_exercises WHERE template_id = ?', [templateId])
...
function createTemplateShell(req, overrides = {}) {
  const body = req.body || {}
  const id = nanoid()
  const now = nowIso()
...
router.post('/', authRequired, (req, res) => {
  db.exec('BEGIN')
  try {
    const created = createTemplateShell(req)
    if (created.error) {
      db.exec('ROLLBACK')
      return res.status(400).json({ error: created.error })
    }
    replaceTemplateExercises(created.id, req.body?.exercises)
    db.exec('COMMIT')
    res.json({ template: loadTemplateFull(created.id) })
  } catch (err) {
...
router.post('/drafts', authRequired, (req, res) => {
  db.exec('BEGIN')
  try {
    const created = createTemplateShell(req, { status: 'draft', name: req.body?.name || 'Untitled template' })
    replaceTemplateExercises(created.id, req.body?.exercises)
    db.exec('COMMIT')
    res.json({ template: loadTemplateFull(created.id) })
  } catch (err) {
...
function canReadWorkout(workout, userId) {
  if (!workout) return false
  if (workout.user_id === userId) return true
  if (workout.visibility === 'public') return true
  if (workout.visibility === 'followers') {
    return !!getOne('SELECT 1 AS x FROM follows WHERE follower_id = ? AND following_id = ?', [userId, workout.user_id])
  }
  return false
}

function workoutToExercises(workoutId) {
  const sets = getAll(
...
router.post('/drafts/from-workout/:wid', authRequired, (req, res) => {
  const workout = getOne('SELECT * FROM workouts WHERE id = ?', [req.params.wid])
  if (!canReadWorkout(workout, req.user.id)) return res.status(workout ? 403 : 404).json({ error: workout ? 'Forbidden' : 'Workout not found' })
  db.exec('BEGIN')
  try {
    const created = createTemplateShell(req, {
      status: 'draft',
    ...
    replaceTemplateExercises(created.id, workoutToExercises(req.params.wid))
    db.exec('COMMIT')
    res.json({ template: loadTemplateFull(created.id) })
  } catch (err) {
    try { db.exec('ROLLBACK') } catch { /* noop */ }
...
router.patch('/:id', authRequired, (req, res) => {
  const current = getOne('SELECT * FROM workout_templates WHERE id = ?', [req.params.id])
  ...
  try {
    ...
    if (Array.isArray(body.exercises)) replaceTemplateExercises(req.params.id, body.exercises)
    db.exec('COMMIT')
    res.json({ template: loadTemplateFull(req.params.id) })
  } catch (err) {
    try { db.exec('ROLLBACK') } catch { /* noop */ }
...
router.delete('/:id', authRequired, (req, res) => {
  ...
  try {
    runQuery('DELETE FROM workout_templates WHERE id = ?', [req.params.id])
    res.json({ ok: true })
  } catch (err) {
...
router.post('/from-workout/:wid', authRequired, (req, res) => {
  const workout = getOne('SELECT * FROM workouts WHERE id = ?', [req.params.wid])
  ...
  try {
    ...
    db.exec('COMMIT')
    res.json({ template: loadTemplateFull(id) })
  } catch (err) {
    try { db.exec('ROLLBACK') } catch { /* noop */ }
