...
router.post('/query', authRequired, (req, res) => {
...
router.post('/compare-cohorts', authRequired, (req, res) => {
  ...
  const a = runQuery(db, { filters: cohortA.filters || [], groupBy, measure, exerciseId, muscle, minCohort })
  const b = runQuery(db, { filters: cohortB.filters || [], groupBy, measure, exerciseId, muscle, minCohort })
...
router.post('/scan', authRequired, (req, res) => {
  const {
    filters,
    groupBys,
...
router.post('/compare-scan', authRequired, (req, res) => {
...
router.get('/findings', authRequired, (_req, res) => {
  const rows = getAll('SELECT * FROM findings ORDER BY discovered_at DESC LIMIT 20')
...
router.get('/saved-questions', authRequired, (req, res) => {
  const rows = getAll(
...
router.post('/saved-questions', authRequired, (req, res) => {
  ...
  const row = getAll('SELECT * FROM research_saved_questions WHERE id = ? AND user_id = ?', [id, req.user.id])[0]
  res.status(201).json({ savedQuestion: savedQuestion(row) })
})

router.delete('/saved-questions/:id', authRequired, (req, res) => {
  dbRunQuery('DELETE FROM research_saved_questions WHERE id = ? AND user_id = ?', [req.params.id, req.user.id])
  res.json({ deleted: true })
})

router.post('/run-batch', authRequired, (req, res) => {
  ...
  const summary = runWeeklyBatch()
  const findings = runFindingsBatch()
