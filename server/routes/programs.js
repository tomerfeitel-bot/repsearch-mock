...
function proofSummaries(programIds) {
...
function withProof(rows) {
  const proofs = proofSummaries(rows.map(row => row.id))
  return rows.map(row => ({ ...row, proof: proofs.get(row.id) || buildProof() }))
}

function proofSummary(programId) {
  return proofSummaries([programId]).get(programId) || buildProof()
}

function loadProgramFull(programId, userId) {
  const p = getOne(
  ...
  return { ...p, blocks, workouts, enrollment, phase, proof: proofSummary(programId) }
}

function canUseTemplate(templateId, userId) {
  if (!templateId) return false
  const t = getOne('SELECT id, user_id, visibility, status FROM workout_templates WHERE id = ?', [templateId])
  return !!t && t.status !== 'draft' && (t.visibility === 'public' || t.user_id === userId)
}

function createProgramShell(userId, body = {}, status = 'draft') {
  const id = nanoid()
  const now = nowIso()
  runQuery(
...
function replaceProgramStructure(programId, userId, blocks = []) {
  runQuery('DELETE FROM program_workouts WHERE program_id = ?', [programId])
  runQuery('DELETE FROM program_blocks WHERE program_id = ?', [programId])
  ...
  safeBlocks.forEach((block, blockIdx) => {
    ...
    sessions.forEach((session, i) => {
      ...
      if (!canUseTemplate(templateId, userId)) return
...
function firstSession(programId) {
  return getOne(
    `SELECT pw.*
       FROM program_workouts pw
...
function activeProgramNext(userId) {
  const enrollment = getOne(
  ...
  const sessions = orderedProgramSessions(enrollment.program_id)
  ...
  if (!phase) {
    ...
    phase = getOne('SELECT * FROM user_program_phase WHERE id = ?', [phaseId])
  }
...
function completedProgramLanding(userId) {
  const enrollment = getOne(
    `SELECT pe.*, p.name AS program_name, p.description AS program_description,
...
router.get('/active/next', authRequired, (req, res) => {
  res.json(activeProgramNext(req.user.id) || completedProgramLanding(req.user.id) || { program: null, phase: null, next_session: null })
})
...
router.get('/', authRequired, (req, res) => {
  ...
  if (status === 'draft') {
    ...
    return res.json({ programs: withProof(rows), sort, status })
  }
  ...
  res.json({ programs: withProof(rows), sort })
})

router.get('/:id', authRequired, (req, res) => {
  const program = loadProgramFull(req.params.id, req.user.id)
  if (!program) return res.status(404).json({ error: 'Program not found' })
  if (program.forbidden) return res.status(403).json({ error: 'Forbidden' })
  res.json({ program })
})

router.post('/drafts', authRequired, (req, res) => {
  const id = createProgramShell(req.user.id, req.body || {}, 'draft')
  if (!getAll('SELECT id FROM program_blocks WHERE program_id = ?', [id]).length) {
    replaceProgramStructure(id, req.user.id, [{ name: 'Main block', sessions: [] }])
  }
  res.json({ program: loadProgramFull(id, req.user.id) })
})
...
router.post('/', authRequired, (req, res) => {
  ...
  db.exec('BEGIN')
  try {
    const id = createProgramShell(req.user.id, body, 'final')
    replaceProgramStructure(id, req.user.id, body.blocks || legacyBlocks(body.workouts))
    db.exec('COMMIT')
    res.json({ program: loadProgramFull(id, req.user.id) })
  } catch (err) {
...
router.patch('/:id', authRequired, (req, res) => {
  const current = getOne('SELECT * FROM programs WHERE id = ?', [req.params.id])
  ...
  try {
    ...
    if (Array.isArray(body.blocks)) replaceProgramStructure(req.params.id, req.user.id, body.blocks)
    db.exec('COMMIT')
    res.json({ program: loadProgramFull(req.params.id, req.user.id) })
  } catch (err) {
    try { db.exec('ROLLBACK') } catch { /* noop */ }
...
router.delete('/:id', authRequired, (req, res) => {
  const current = getOne('SELECT id, user_id FROM programs WHERE id = ?', [req.params.id])
  if (!current) return res.status(404).json({ error: 'Program not found' })
  if (current.user_id !== req.user.id) return res.status(403).json({ error: 'Forbidden' })
  runQuery('DELETE FROM programs WHERE id = ?', [req.params.id])
  res.json({ ok: true })
})

router.post('/:id/start', authRequired, (req, res) => {
  const program = getOne('SELECT * FROM programs WHERE id = ?', [req.params.id])
  if (!program) return res.status(404).json({ error: 'Program not found' })
  ...
  const session = firstSession(req.params.id)
...
router.post('/:id/phase-decision', authRequired, (req, res) => {
  ...
  res.json({ phase: getOne('SELECT * FROM user_program_phase WHERE id = ?', [phase.id]) })
})
...
router.get('/:id/results', authRequired, (req, res) => {
  ...
  const rows = getAll(
    `SELECT uep.exercise_id, e.name AS exercise_name, e.primary_muscle,
      ...
      WHERE pe.program_id = ?
      GROUP BY uep.exercise_id
      HAVING n_users >= 3
      ORDER BY avg_progression_rate DESC`,
    [req.params.id],
  )
  const enrollCount = getOne(
...
router.get('/:id/evidence', authRequired, (req, res) => {
  const program = getOne('SELECT * FROM programs WHERE id = ?', [req.params.id])
  if (!program) return res.status(404).json({ error: 'Program not found' })
  if (program.visibility !== 'public' && program.user_id !== req.user.id) {
    return res.status(403).json({ error: 'Forbidden' })
  }
  const proof = proofSummary(req.params.id)
  const exactUsers = getOne(
    `SELECT COUNT(DISTINCT user_id) AS n FROM workouts WHERE program_id = ? AND run_classification = 'exact'`,
    [req.params.id],
  ).n
  const adaptedUsers = getOne(
