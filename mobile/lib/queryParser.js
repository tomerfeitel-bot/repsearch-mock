...
// explorer's slots â€” groupBy, measure, filters, mode, plus exercise/muscle scope â€”
...
import { describeQuery, prettyBucket, prettyGroupBy, prettyMeasure } from './researchTheme'
...
function fuzzyHit(term, tokens) {
  ...
  // so only fuzzy-match terms of 5+ chars. protienâ†’protein (len 7) still works.
...
function compose(text) {
  ...
  // No axis yet, but a lone filter on a groupable dimension â†’ make it the axis.
  ...
  // Still no axis but we have scope/filters â†’ fall back to the default axis.
