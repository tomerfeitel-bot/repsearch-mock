// Full port of src/lib/researchTheme.js (Session 5 — the Session-2 trim kept
// only the group-by/measure whitelists for the post composer). Mirrors the
// whitelists in server/research/queryEngine.js so the UI can't construct
// queries the backend will reject.

// Study shares the system ground/surface tokens; its signature is the Dark
// Jewel pine/teal, kept mid-toned.
...
export type ResearchFilter = { field: string; op: string; value?: string | number };
export type FieldMeta = { value: string; label: string; enum?: string[]; type?: string };
...
export const FIELD_OPTIONS: { group: string; fields: FieldMeta[] }[] = [
...
export const FIELD_BY_VALUE: Record<string, FieldMeta> = (() => {
...
// The Measure step shows only this curated outcome set. MEASURE_OPTIONS stays
// broader so saved queries still resolve labels.
...
export const PERSONAL_BUCKET_FROM_USER: Record<string, (u: UserLike) => string | null | undefined> = {
...
export function prettyBucket(value?: string | number | null): string {
  if (value === null || value === undefined) return '—';
  return String(value).replace(/_/g, ' ');
}

export function prettyField(value: string): string {
...
// Topic styles drawn from the Dark Jewel palette. `fill` — solid pigment for
// badges/markers; `on` — contrasting ink on fill; `color` — lifted ink for
// text/lines on the dark ground; `tint` — fill at low alpha, data-viz only.
export type StudyTopic = { label: string; symbol: string; fill: string; on: string; color: string; tint: string };

export const STUDY_TOPIC_STYLES: Record<string, StudyTopic> = {
...
const STUDY_TOPIC_BY_GROUP: Record<string, string> = {
...
export function studyTopicForQuery(query: Record<string, any> = {}, type?: string): StudyTopic {
...
export function studyChipsForQuery(query: Record<string, any> = {}, type?: string): string[] {
  const topic = studyTopicForQuery(query, type);
  const chips: string[] = [topic.label];
...
export type MatchValue = { minus: number; plus: number } | null;
export type PeopleFilter = {
  key: string;
  label: string;
  matchKind: 'exact' | 'number';
  unit?: string;
  step?: number;
  defaultMinus?: number;
  defaultPlus?: number;
  userValue: (u?: UserLike | null) => string | null | undefined;
  toFilters: (u: UserLike | null | undefined, value?: MatchValue) => ResearchFilter[];
};
...
export const PEOPLE_FILTER_BY_KEY: Record<string, PeopleFilter> = PEOPLE_FILTERS.reduce(
  (acc, f) => ({ ...acc, [f.key]: f }),
  {} as Record<string, PeopleFilter>,
);
export const DEFAULT_MATCH_KEYS = ['split_type', 'experience_level', 'gender'];

export function defaultMatchValue(filter?: PeopleFilter | null): MatchValue {
...
export function peopleToFilters(
  user: UserLike | null | undefined,
  matchKeys: string[] = [],
  matchValues: Record<string, MatchValue> = {},
): ResearchFilter[] {
  if (!user) return [];
  const out: ResearchFilter[] = [];
...
export type ResearchBucket = { label: string; avg_measure: number | null; n: number; sd?: number | null };

// Label the shape of a bucket series for the result detail panel.
export function detectPattern(buckets?: ResearchBucket[] | null): string {
  const vals = (buckets || []).map((b) => b.avg_measure).filter((v): v is number => v != null);
